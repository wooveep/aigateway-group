...
- [x] `P4-AF-04`：废弃父账号 / 子账号管理模型，统一收口为“部门树 + 部门管理员 + 部门成员”，并同步清理 Portal / Console 前后端契约中的 `parentConsumerName` 语义。
...
- 本轮额外收口组织管理模型：
  - Portal 新增 `POST /api/accounts/managed`，部门管理员只能在自己当前部门下创建成员账号；密码留空时后端自动生成临时密码返回。
  - Portal 成员余额调整改为管理员钱包与成员钱包之间的强一致转账，正数表示管理员转出给成员，负数表示成员退回管理员；两边钱包与流水在单事务内同时写入。
  - Console `POST /v1/org/departments` 改为“建部门 + 建管理员账号”原子事务，`PUT /v1/org/departments/:departmentId` 支持改绑到当前部门内活跃账号。
  - Console/Portal 前后端类型、页面文案和 assignment 接口已移除 `parentConsumerName`；共享 schema 迁移会将历史 `org_account_membership.parent_consumer_name` 全量清空并停用读写。
