...
- [x] `P3-AF-07`：梳理 `Console model-assets / ai-providers / Portal / ai-proxy` 的协议事实源，固定一级协议为 `openai/v1`、`anthropic/v1/messages`、`original`，并把 `responses` 等能力降级为 capability / 推荐入口。
- [x] `P3-AF-08`：固定 `ai-proxy` 协议优先级；`provider.protocol=""` 保持自动检测，显式 `openai` 仍允许 Claude `/v1/messages` 自动兜底，显式 `anthropic` 固定按 Claude `messages` 入口处理，显式 `original` 才完全关闭 OpenAI / Claude 自动检测。
- [x] `P3-AF-09`：建立 `Console` 的协议/入口推荐目录，统一输出 `protocolOptions`、`recommendedEndpoints`、`providerProtocolMatrix`、`providerDocsStatus`；`protocol` 仅表达一级协议概念，`capability` 用于记录 `responses` 等已覆盖能力。
- [x] `P3-AF-10`：按供应商核对原生协议、OpenAI 兼容协议、Claude 兜底能力、推荐入口地址与转换前提；其中 `responses` 不单列为协议，只作为 capability 记录。
- [x] `P3-AF-11`：收口并索引既有 `TASK/projects/aigateway-console/provider-api-docs/`；后续所有新增协议判断、推荐入口与转换策略都必须以该目录下各 provider 子目录中的官方 API 文档为准，未投递文档的供应商协议统一标记为待确认。
...
- 本轮已完成协议事实源与 `ai-proxy` 协议收口：
  - 一级 canonical protocol 已固定为 `openai/v1`、`anthropic/v1/messages`、`original`；Provider 资源额外允许 `auto`，用于映射 `ai-proxy` 的空协议自动检测模式。
  - `ai-proxy` 继续保留 OpenAI / Claude 自动协议检测能力；显式 `openai` 仍允许 Claude `/v1/messages` 自动兜底，显式 `anthropic` 固定按 Claude `messages` 入口处理，显式 `original` 才完全关闭自动检测。
  - `responses` 不再单列为一级协议；若当前 `ai-proxy` 已支持 `/v1/responses`，统一按 OpenAI 协议内 capability 与推荐入口处理。
  - `Console` 已新增协议目录输出，统一返回 `protocolOptions`、`recommendedEndpoints`、`providerProtocolMatrix`、`providerDocsStatus`，并按 provider 维度区分原生协议、OpenAI 兼容协议、Claude 兜底能力与推荐入口。
  - 所有新增供应商协议判断统一以下述目录中的 provider 子目录官方文档为准：`TASK/projects/aigateway-console/provider-api-docs/<provider>/`。
- 最近新增 aftercare 第二批：
  ...
  - `P3-AF-07 ~ P3-AF-11` 本轮已落地：`Console / Portal / ai-proxy` 已统一收口 canonical protocol、推荐入口和 capability 边界；Provider 编辑页与 Model Binding 编辑页改为受控协议下拉，协议目录以 `provider-api-docs/<provider>/` 子目录中的官方文档和 `ai-proxy` 源码为双真相源；`provider.protocol=openai` 继续保留 Claude `/v1/messages` 自动兜底，`provider.protocol=original` 才完全关闭自动检测。
