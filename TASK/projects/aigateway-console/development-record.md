...
- `P1` 组织与授权底座收口
- Portal / Console 组织账号模型修订
- “父账号 / 子账号” -> “部门树 + 部门管理员 + 部门成员”

### 改动摘要

- Portal 与 Console 正式废弃父账号 / 子账号管理模型，统一改为基于 `org_department` 子树和 `admin_consumer_name` 的部门管理模型。
- Portal 后端新增 `POST /api/accounts/managed`：
  - 仅部门管理员可调用；
  - 新建账号固定落在操作者当前部门；
  - 不再读写 `parent_consumer_name`；
  - 密码留空时自动生成临时密码并返回给前端复制。
- Portal 成员余额调整改为“管理员钱包 <-> 成员钱包”强一致转账：
  - `amount > 0` 时从管理员余额扣减并给成员加款；
  - `amount < 0` 时从成员余额回收并退回管理员；
  - `portal_balance_adjustment`、两侧 `billing_wallet`、两条 `billing_transaction` 在单事务内完成，任一侧余额不能为负。
- Console 部门管理改为“建部门即建管理员”：
  - `POST /v1/org/departments` 必须同时创建新的管理员账号和 membership；
  - `PUT /v1/org/departments/:departmentId` 仅允许把管理员改绑到当前部门内 `active` 账号；
  - `POST/PUT/PATCH /v1/org/accounts*` 已移除 `parentConsumerName`。
- 共享 schema / 迁移语义收口：
  - 历史 `org_account_membership.parent_consumer_name` 统一清空；
  - 保留列本身以降低迁移风险，但前后端和服务层均已停用其读写语义。
- 前端页面与文案同步收口：
  - Portal “子账号管理”改为部门成员管理；
  - Console `/consumer` 页面改为“新建部门填写管理员账号 + 编辑部门改绑管理员”；
  - 所有前端类型和接口移除 `parentConsumerName`。

### 关键文件 / 接口

- `aigateway-portal/backend/internal/service/portal/managed_accounts.go`
- `aigateway-portal/backend/internal/service/portal/{auth,organization,scope_helpers}.go`
- `aigateway-portal/backend/schema/shared/shared.go`
- `aigateway-portal/frontend/src/pages/ManagedAccountsPage.vue`
- `aigateway-console/backend/internal/service/portal/service.go`
...
- `aigateway-console/frontend/src/views/consumer/ConsumerPage.vue`
- `/api/accounts/managed`
- `/api/accounts/:consumerName/balance-adjustments`
- `/v1/org/departments`
- `/v1/org/accounts/:consumerName/assignment`

### 验证结果

- `cd aigateway-portal/backend && go test ./internal/service/portal/... ./internal/controller/portal/... ./internal/model/... ./schema/shared/...` 通过。
- `cd aigateway-portal/frontend && npm run build` 通过。
- `cd aigateway-console/backend && go test ./internal/service/portal/... ./internal/controller/portal/... ./utility/clients/portaldb/...` 通过。
- `cd aigateway-console/frontend && npm run build` 通过。

### 与旧实现的差异

- 旧“父账号管理子账号”语义不再是正式模型，后续所有组织管理、账单 scope、API Key scope 都只以“部门管理员是否管理该部门树成员”为准。
- `parent_consumer_name` 仅作为历史兼容列保留，不再作为业务判断条件；新增代码不得继续依赖该字段。
...
- `TASK/projects/aigateway-console/java-go-parity-matrix.md`
...
- `TASK/projects/aigateway-console/{README.md,P3-gateway-resource-domain.md,java-go-parity-matrix.md,development-record.md}`
