# AIGateway Console 专项任务台账

> 文档职责：这里收口 Console 的长篇专项材料。  
> `aigateway-console/` 子项目根目录只保留 `README.md`、`project.md`、`TODO.md`、`TASK.md` 等轻量入口文档。
...
- `P3-AF-07 ~ P3-AF-11` 已完成：`Console model-assets / ai-providers / Portal / ai-proxy` 的协议事实源已统一收口；一级 protocol 固定为 `openai/v1`、`anthropic/v1/messages`、`original`，Provider 资源额外允许 `auto`；`responses` 等仅作为 capability / 推荐入口；Console 已新增协议目录接口与 provider 矩阵，并以 `provider-api-docs/<provider>/` 子目录中的官方文档和 `ai-proxy` 源码作为双真相源。
- 本地开发链路最近已补齐：
...
- `P3`
  ...
  - 继续扩展 `providerProtocolMatrix` 与 provider docs 覆盖范围，但所有新增协议判断、推荐入口与转换策略都必须以 `provider-api-docs/<provider>/` 子目录中的官方 API 文档为前提；当前已固定语义为“`auto` 保留自动检测，显式 `openai` 仍允许 Claude `/v1/messages` 兜底，显式 `original` 才关闭自动检测”。
- `P5`
  - 补失败重试与更完整的任务运行验证。
